...
def evaluate_route(route: List[Dict]) -> Tuple[float, float]:
    """Evaluate route based on successful deliveries and total distance."""
    ...
    for delivery in route:
        ...
        if driver_id not in current_locations:
            # Initialize from driver's depot location
            current_locations[driver_id] = delivery["driver_depot"]
            
        distance = calculate_distance(current_locations[driver_id], f"{delivery['address']}, {delivery['city']}, {delivery['country']}")
        total_distance += distance
        current_locations[driver_id] = f"{delivery['address']}, {delivery['city']}, {delivery['country']}"
    
    return success_count, -total_distance
...
def optimize_route(drivers: List[Dict], deliveries: List[Dict], iterations: int = 10) -> List[Dict]:
    ...
    """
    best_route = []
    best_score = (-1, float('-inf'))  # (deliveries, -distance)
...
def optimize_single_route(drivers: List[Dict], deliveries: List[Dict]) -> List[Dict]:
    ...
    # Initialize drivers with their depot locations as current locations
    for driver in drivers:
        driver['current_location'] = driver['depot_location']
    ...
    for delivery in deliveries:
        ...
        # Atjaunina šoferu darba laiku un atrašanās vietu
        best_driver['current_location'] = delivery['full_address']
        
        # Atjaunina šoferu kopējo darba laiku
        best_driver['total_working_time'] = best_arrival_time - shift_start + stop_time
        ...
        optimized_route.append({
            ...
            "driver_depot": best_driver["depot_location"],
            "order": len(optimized_route) + 1,
...
def main():
    # Test data
    drivers = [
        {'driver_id': '1001', 'first_name': 'Artjoms', 'last_name': 'Šefanovskis', 'phone_number': '12345678', 'depot_location': "Lucavsalas iela 3, Zemgales priekšpilsēta, Rīga, LV-1004", 'priority': 1},
        {'driver_id': '1002', 'first_name': 'Niklāvs', 'last_name': 'Zebinskis', 'phone_number': '87654321', 'depot_location': "Lucavsalas iela 3, Zemgales priekšpilsēta, Rīga, LV-1004", 'priority': 2},
        {'driver_id': '1003', 'first_name': 'Sergejs', 'last_name': 'Zembkovskis', 'phone_number': '22222222', 'depot_location': "Lucavsalas iela 3, Zemgales priekšpilsēta, Rīga, LV-1004", 'priority': 3}
    ]
    ...
    for assignment in optimized_route:
        ...
        if driver_id not in driver_stats:
            driver_stats[driver_id] = {
                'deliveries': 0,
                'distance': 0,
                'last_location': drivers[next(i for i, d in enumerate(drivers) if d['driver_id'] == driver_id)]['depot_location']
            }
        
        # Calculate distance from last location (depot or previous delivery)
        current_address = f"{assignment['address']}, {assignment['city']}, {assignment['country']}"
        distance = calculate_distance(driver_stats[driver_id]['last_location'], current_address)
        
        driver_stats[driver_id]['distance'] += distance
        driver_stats[driver_id]['last_location'] = current_address
        driver_stats[driver_id]['deliveries'] += 1
        total_distance += distance
            
    for driver_id, stats in driver_stats.items():
        print(f"\nDriver {driver_id}:")
        print(f"Completed deliveries: {stats['deliveries']}")
        print(f"Total distance driven: {stats['distance']/1000:.2f} km")
    
    print(f"\nTotal combined distance driven: {total_distance/1000:.2f} km")
